...
<div class="main-container">
    ...
    <div class="content-area">
        <h2>我的维修申请</h2>
        <div class="actions">
            <a href="${pageContext.request.contextPath}/owner/repair?action=apply" class="btn btn-primary">提交维修申请</a>
        </div>
        <% if (list == null || list.isEmpty()) { %>
            <p style="color:#999;margin-top:30px;">暂无维修申请记录。</p>
        <% } else { %>
        <table>
            <tr>
                <th>申请ID</th><th>地址</th><th>维修类型</th><th>问题描述</th><th>申请时间</th><th>状态</th><th>操作</th>
            </tr>
            <% for (RepairRequest rr : list) {
                ...
                switch (rr.getStatus()) {
                    ...
                    case "待确认": badgeClass = "badge-info"; break;
                }
            %>
            <tr>
                ...
                <td><%=rr.getDescription() != null && rr.getDescription().length() > 15 ? rr.getDescription().substring(0,15)+"..." : rr.getDescription()%></td>
                <td><%=rr.getApplyTime() != null ? rr.getApplyTime().substring(0,16) : ""%></td>
                <td><span class="badge <%=badgeClass%>"><%=rr.getStatus()%></span></td>
                <td>
                    <a href="${pageContext.request.contextPath}/owner/repair?action=detail&requestId=<%=rr.getRequestId()%>" class="btn btn-info">查看详情</a>
                    <% if ("待审核".equals(rr.getStatus())) { %>
                        <a href="${pageContext.request.contextPath}/owner/repair?action=cancel&requestId=<%=rr.getRequestId()%>" class="btn btn-danger" onclick="return confirm('确定要取消该申请吗？')">取消</a>
                    <% } %>
                </td>
            </tr>
            <% } %>
